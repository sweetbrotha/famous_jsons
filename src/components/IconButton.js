...
const IconButton = ({ name, onClick, disabled, isPressed = false }) => {
  ...
  return (
    ...
    >
      <button
        className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-opacity-50 ${
          (isPressed || disabled) ? 'bg-mediumgray' : 'bg-lightgray'
        }`}
        style={{ boxShadow: 'none' }}
        disabled={disabled}
      >
        <img src={iconPath} alt={name} className={`w-3 h-3 md:w-4 md:h-4 ${isPressed ? 'opacity-100' : 'opacity-80'}`} />
      </button>
      {isHovered && !disabled && (
